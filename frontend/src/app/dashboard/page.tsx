...
interface UrlFormValues extends Record<string, unknown> {
  longUrl: string;
}
...
export default function Dashboard() {
  ...
  const recentUrls =
    urls?.filter((url) => new Date(url.createdAt) >= sevenDaysAgo) || [];
  ...
  const handleSelectAll = (selected: boolean) => {
    setSelectedUrls(selected ? urls?.map((url) => url.id) || [] : []);
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white/80 backdrop-blur-sm rounded-lg shadow-xl ring-1 ring-indigo-900/5 p-6">
            ...
            <p className="text-xs text-indigo-500 mt-1">
              {recentUrls.length} created in the last 7 days
            </p>
          </div>
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-lg shadow-xl ring-1 ring-indigo-900/5 p-6">
            ...
            <p className="text-xs text-indigo-500 mt-1">
              Avg. {avgClicksPerUrl} clicks per URL
            </p>
          </div>
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-lg shadow-xl ring-1 ring-indigo-900/5 p-6">
            ...
            <div className="text-2xl font-bold text-indigo-900">
              {topUrls.length > 0 ? topUrls[0].clicks : 0} clicks
            </div>
            <p className="text-xs text-indigo-500 mt-1 truncate">
              {topUrls.length > 0 ? topUrls[0].longUrl : "No URLs yet"}
            </p>
          </div>
        </div>
